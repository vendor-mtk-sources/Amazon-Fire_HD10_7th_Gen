...
static const char * const life_cycle_reasons[] = {
	...
	"SOC Overheated Thermal System Reset",
...
static int life_cycle_reason_lookup(void)
{
	...
	life_cycle_special_mode s_mode = (life_cycle_special_mode)(-1);
	...
	case THERMAL_SHUTDOWN_REASON_PCB:
		...
		lcr_found = true;
		break;
	case THERMAL_SHUTDOWN_REASON_BTS:
		p_dev_sol->life_cycle_reason_idx = BTS_THERMAL_SHUTDOWN;
		lcr_found = true;
		break;
	case THERMAL_SYSRST_SOC:
		p_dev_sol->life_cycle_reason_idx = SOC_THERMAL_SYSRST;
		lcr_found = true;
		break;
	default:
		break;
	...
	case SHUTDOWN_BY_SW_LONG_PWR_KEY_PRESS:
		p_dev_sol->life_cycle_reason_idx = LONG_KEY_PRESSED_PWR_KEY_SHUTDOWN;
		lcr_found = true;
		printk(KERN_NOTICE"%s: life cycle reason is SW Long Pressed Power Key Shutdown\n", __func__);
		break;
	...
	case LIFE_CYCLE_SMODE_LOW_BATTERY:
		p_dev_sol->life_cycle_reason_idx = LOW_BATTERY_SHUTDOWN;
		lcr_found = true;
		break;
	default:
		break;
