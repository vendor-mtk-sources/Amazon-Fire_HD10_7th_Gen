...
struct thermal_dev_params *tmp103_dt_to_params(
                                struct device *dev)
{
        ...
        of_property_read_u32(node, "tmp103-sensor,offset",
                                        &params->offset);
        of_property_read_u32(node, "tmp103-sensor,alpha",
                                        &params->alpha);
        of_property_read_u32(node, "tmp103-sensor,weight",
                                        &params->weight);
...
static int tmp103_temp_sensor_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (tmp103->therm_fw) {
		...
		tmp103->therm_fw->tdp = tmp103_dt_to_params(&client->dev);
