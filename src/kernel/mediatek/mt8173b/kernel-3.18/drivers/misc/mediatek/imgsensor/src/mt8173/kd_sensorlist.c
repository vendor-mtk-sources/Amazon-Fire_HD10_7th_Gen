...
#ifdef CONFIG_OF
#define DT_NOT_SUPPORT  16
#endif
int camera_i2c_bus_num1 = 3;
int camera_i2c_bus_num2 = 6;
...
#define PK_WARN(fmt, arg...) pr_err(PFX "[%s] Warning:" fmt, __func__, ##arg)
...
static ssize_t RFC_id_get(struct device *dev, struct device_attribute *attr, char *buf)
{
    return sprintf(buf, "%d\n", RFC_id_value);
}

static ssize_t FFC_id_get(struct device *dev, struct device_attribute *attr, char *buf)
{
    return sprintf(buf, "%d\n", FFC_id_value);
}

static DEVICE_ATTR(TFlashSwitch, 0644, TFlashSwitch_get, TFlashSwitch_set);
static DEVICE_ATTR(RFC_id, 0444, RFC_id_get, NULL);
static DEVICE_ATTR(FFC_id, 0444, FFC_id_get, NULL);

static struct attribute *dev_attrs[] = {
	&dev_attr_TFlashSwitch.attr,
	&dev_attr_RFC_id.attr,
	&dev_attr_FFC_id.attr,
	NULL
};
...
static u32 of_get_cam_bus(struct device *dev)
{
	...
	if (parent == NULL) {
		return DT_NOT_SUPPORT;
	}
	err = of_property_read_u32(parent, "cell-index", &bus);
	if (err) {
		PK_DBG("[CAMERA_HW] %s: failed to read cell-index\n", parent->full_name);
		return DT_NOT_SUPPORT;
	}

	/* PK_DBG("[CAMERA_HW] Get bus num %d from DT\n", bus); */
...
#ifdef CONFIG_OF
	if (of_get_cam_bus(&g_pstI2Cclient->dev) != DT_NOT_SUPPORT)
		camera_i2c_bus_num1 = of_get_cam_bus(&g_pstI2Cclient->dev);
	PK_DBG("[CAMERA_HW] I2C bus %d\n", camera_i2c_bus_num1);
#endif
	spin_unlock(&kdsensor_drv_lock);
...
#ifdef CONFIG_OF
	if (of_get_cam_bus(&g_pstI2Cclient2->dev) != DT_NOT_SUPPORT)
		camera_i2c_bus_num2 = of_get_cam_bus(&g_pstI2Cclient2->dev);
	PK_DBG("[CAMERA_HW] I2C bus %d\n", camera_i2c_bus_num2);
#endif
	spin_unlock(&kdsensor_drv_lock);
