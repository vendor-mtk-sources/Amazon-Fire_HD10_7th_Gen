...
static DECLARE_WORK(long_press_restart_work, long_press_deferred_restart);
...
static irqreturn_t thr_h_int_handler(int irq, void *dev_id)
{
	...
	if (val == 0) {
		...
		sd_thermal_detect_change();
		pr_err("%s: VMCH not good with status: 0x%x, turn off!\n",
			__func__, upmu_get_reg_value(TEST_OUT_H));
	}
...
#endif
		ktime = ktime_set(POWER_KEY_PRESS_CHECK_TIME, 0);
...
#if defined(CONFIG_MTK_ACCDET) || defined(CONFIG_ASUS_MT8173_ACCDET)
...
static int mt6397_irq_handler_init(struct mt6397_chip_priv *chip)
{
	...
	for (i = 0; i < ARRAY_SIZE(mt6397_irqs); i++) {
		...
		pr_debug("%s: registered irq=%d (%d); name='%s'; enabled: %d\n",
...
void PMIC_INIT_SETTING_V1(void)
{
	...
	ret = pmic_config_interface(0x50A, 0x1, 0x1, 10);	/* [10]: VMCH_PG_ENB; */
	ret = pmic_config_interface(0x432, 0x0, 0x1, 6);	/* [6]: RG_VMCH_STB_SEL; */
	ret = pmic_config_interface(0x44E, 0x0, 0x1, 15);	/* [15]: RG_STB_SEL; */
	ret = pmic_config_interface(0x432, 0x1, 0x1, 2);	/* [2]: RG_VMCH_OCFB; */
}
...
void PMIC_CUSTOM_SETTING_V1(void)
{
	...
	upmu_set_rg_dcxo_ldo_rf2_reg_en(0x0);
