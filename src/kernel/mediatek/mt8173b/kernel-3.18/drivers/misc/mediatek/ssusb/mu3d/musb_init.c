...
static int mtu3d_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(pinctrl))
		mu3d_dbg(K_ERR, "fail to get pinctrl\n");

	pinctrl_iddig_high = pinctrl_lookup_state(pinctrl, "iddig_high");

	if (IS_ERR(pinctrl_iddig_high)) {
		ret = PTR_ERR(pinctrl_iddig_high);
		mu3d_dbg(K_ERR, "fail to get pinctrl_iddig_high\n");;
	}

	pinctrl_iddig_low = pinctrl_lookup_state(pinctrl, "iddig_low");

	if (IS_ERR(pinctrl_iddig_low)) {
		ret = PTR_ERR(pinctrl_iddig_low);
		mu3d_dbg(K_ERR, "fail to get pinctrl_iddig_low\n");;
	}
...
static int mtu3d_suspend(struct device *dev)
{
	...
	disable_irq(otg_switch->iddig_eint_num);
