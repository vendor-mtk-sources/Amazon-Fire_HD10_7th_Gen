...
#ifdef CONFIG_AMAZON_SIGN_OF_LIFE
		tscpu_printk("[%s] Thermal system reset\n",
			__func__);
		life_cycle_set_thermal_shutdown_reason(THERMAL_SYSRST_SOC);
#endif
			   ...
			   loff_t *data)
{
	...
	if (sscanf
		...
		if (num_trip < 0 || num_trip > 10) {
			pr_debug("[mtkts_cpu_write] bad argument\n");
			return -EINVAL;
		}
