...
static int cmdq_open(struct inode *pInode, struct file *pFile)
{
	...
	if (NULL == pFile->private_data) {
		CMDQ_ERR_PRINT_LIMIT("Can't allocate memory for CMDQ file node\n");
		return -ENOMEM;
	}
...
static int cmdq_release(struct inode *pInode, struct file *pFile)
{
	...
	if (NULL == pNode) {
		CMDQ_ERR_PRINT_LIMIT("CMDQ file node NULL\n");
		return -EFAULT;
	}
...
static void cmdq_driver_process_read_address_request(struct cmdqReadAddressStruct *req_user)
{
	...
	do {
		if (NULL == req_user ||
			...
			CMDQ_ERR_PRINT_LIMIT("[READ_PA] invalid req_user\n");
			break;
		}

		addrs = kcalloc(req_user->count, sizeof(uint32_t), GFP_KERNEL);
		if (NULL == addrs) {
			CMDQ_ERR_PRINT_LIMIT("[READ_PA] fail to alloc addr buf\n");
			break;
		}

		values = kcalloc(req_user->count, sizeof(uint32_t), GFP_KERNEL);
		if (NULL == values) {
			CMDQ_ERR_PRINT_LIMIT("[READ_PA] fail to alloc value buf\n");
			break;
		}
		/* copy from user */
		if (copy_from_user
			...
			CMDQ_ERR_PRINT_LIMIT("[READ_PA] fail to copy user dmaAddresses\n");
			break;
		}
		/* actually read these PA write buffers */
		for (i = 0; i < req_user->count; ++i) {
			pa = (0xFFFFFFFF & addrs[i]);
			CMDQ_ERR_PRINT_LIMIT("[READ_PA] req read dma address 0x%pa\n", &pa);
		...
		if (copy_to_user
			...
			CMDQ_ERR_PRINT_LIMIT("[READ_PA] fail to copy to user value buf\n");
			break;
		}
...
static long cmdq_driver_create_secure_medadata(struct cmdqCommandStruct *pCommand)
{
	...
	if (NULL == pAddrMetadatas) {
		CMDQ_ERR_PRINT_LIMIT("[secData]kzalloc for addrMetadatas failed, count:%d, alloacted_size:%d\n",
	...
	if (copy_from_user(pAddrMetadatas, CMDQ_U32_PTR(pCommand->secData.addrMetadatas), length)) {
		...
		CMDQ_ERR_PRINT_LIMIT("[secData]fail to copy user addrMetadatas\n");
...
static long cmdq_driver_process_command_request(struct cmdqCommandStruct *pCommand)
{
	...
	if (pCommand->regRequest.count != pCommand->regValue.count) {
		CMDQ_ERR_PRINT_LIMIT("mismatch regRequest and regValue\n");
		return -EFAULT;
	}
	...
	if (0 > status) {
		CMDQ_ERR_PRINT_LIMIT("Submit user commands for execution failed = %d\n", status);
		cmdq_driver_destroy_secure_medadata(pCommand);
		kfree(CMDQ_U32_PTR(pCommand->regRequest.regAddresses));
	...
	if (0 != pCommand->debugRegDump) {
		status = cmdqCoreDebugRegDumpEnd(pCommand->debugRegDump,
						 ...
						 userRegCount);
		if (0 != status)
			CMDQ_ERR_PRINT_LIMIT("cmdqCoreDebugRegDumpEnd returns %d\n", status);
	...
	if (userRegValue && userRegCount) {
		...
		if (copy_to_user
		    (userRegValue, CMDQ_U32_PTR(pCommand->regValue.regValues),
		     userRegCount * sizeof(uint32_t))) {
			CMDQ_ERR_PRINT_LIMIT("Copy REGVALUE to user space failed\n");
		}
	}
...
static long cmdq_ioctl(struct file *pFile, unsigned int code, unsigned long param)
{
	...
	case CMDQ_IOCTL_LOCK_MUTEX:
		...
		if (copy_to_user((void *)param, &mutex, sizeof(int))) {
			CMDQ_ERR_PRINT_LIMIT("Copy mutex number to user failed\n");
	...
	case CMDQ_IOCTL_UNLOCK_MUTEX:
		if (copy_from_user(&mutex, (void *)param, sizeof(int))) {
			CMDQ_ERR_PRINT_LIMIT("Copy mutex number from user failed\n");
		...
		break;
	case CMDQ_IOCTL_GET_FENCE_FD:
		status = cmdqFenceGetFence(&fence);
		if (status) {
			CMDQ_ERR_PRINT_LIMIT("get fence failed[%d]\n", status);
			return IOCTL_RET_GET_FENCE__FAIL;
		}

		if (copy_to_user((void *)param, &fence, sizeof(struct cmdqFenceStruct))) {
			CMDQ_ERR_PRINT_LIMIT("Copy fence fd to user failed\n");
			/* release fence */
			if (cmdqFenceReleaseFence(fence))
				return -IOCTL_RET_RELEASE_FENCE_FAIL;

			return -IOCTL_RET_COPY_FENCE_TO_USER_FAIL;
		}
		break;
	case CMDQ_IOCTL_RELEASE_FENCE_FD:
		if (copy_from_user(&fence, (void *)param, sizeof(struct cmdqFenceStruct))) {
			CMDQ_ERR_PRINT_LIMIT("copy fence fd from user filed\n");
			return -IOCTL_RET_COPY_FENCE_FROM_USER_FAIL;
		}

		CMDQ_MSG("release fence fd[%d] value[%d]\n", fence.fenceFd, fence.fenceValue);
		if (cmdqFenceReleaseFence(fence))
				return -IOCTL_RET_RELEASE_FENCE_FAIL;
		break;
	case CMDQ_IOCTL_EXEC_COMMAND:
		...
		if (cmdqCoreIsSuspend()) {
			CMDQ_ERR_PRINT_LIMIT("cmdq driver is suspended\n");
			if (cmdqFenceReleaseFence(command.fence))
				return -IOCTL_RET_RELEASE_FENCE_FAIL;
	...
	case CMDQ_IOCTL_QUERY_USAGE:
		...
		if (copy_to_user((void *)param, count, sizeof(int32_t) * CMDQ_MAX_ENGINE_COUNT)) {
			CMDQ_ERR_PRINT_LIMIT("CMDQ_IOCTL_QUERY_USAGE copy_to_user failed\n");
	...
	case CMDQ_IOCTL_ASYNC_JOB_EXEC:
		...
		if (true == job.command.secData.isSecure) {
			CMDQ_ERR_PRINT_LIMIT("not support secure path for CMDQ_IOCTL_ASYNC_JOB_EXEC\n");
		...
		if (status >= 0) {
			...
			if (copy_to_user((void *)param, (void *)&job, sizeof(struct cmdqJobStruct))) {
				CMDQ_ERR_PRINT_LIMIT("CMDQ_IOCTL_ASYNC_JOB_EXEC copy_to_user failed\n");
	...
	case CMDQ_IOCTL_ASYNC_JOB_WAIT_AND_CLOSE:
		if (copy_from_user(&jobResult, (void *)param, sizeof(jobResult))) {
			CMDQ_ERR_PRINT_LIMIT("copy_from_user jobResult fail\n");
			return -IOCTL_RET_COPY_ASYNC_JOB_WAIT_AND_CLOSE_FROM_USER_FAIL;
		}

		if (cmdqCoreIsSuspend()) {
			CMDQ_ERR_PRINT_LIMIT("CMDQ_IOCTL_ASYNC_JOB_WAIT_AND_CLOSE suspended, return\n");
		...
		if (!cmdqIsValidTaskPtr((struct TaskStruct *)(unsigned long)jobResult.hJob)) {
			CMDQ_ERR_PRINT_LIMIT("invalid task ptr = 0x%llx\n", jobResult.hJob);
		...
		if (jobResult.regValue.count < pTask->regCountUserSpace) {
			...
			if (copy_to_user((void *)param, (void *)&jobResult, sizeof(jobResult))) {
				CMDQ_ERR_PRINT_LIMIT("copy_to_user fail, line=%d\n", __LINE__);
				return -IOCTL_RET_COPY_JOB_RESULT_TO_USER1_FAIL;
			}
			CMDQ_ERR_PRINT_LIMIT("insufficient register buffer\n");
		...
		if (NULL == CMDQ_U32_PTR(jobResult.regValue.regValues)) {
			CMDQ_ERR_PRINT_LIMIT("no reg value buffer\n");
		...
		if (status < 0) {
			CMDQ_ERR_PRINT_LIMIT("waitResultAndReleaseTask fail=%d\n", status);
	...
	case CMDQ_IOCTL_ALLOC_WRITE_ADDRESS:
		do {
			...
			if (copy_to_user((void *)param, &addrReq, sizeof(addrReq))) {
				CMDQ_ERR_PRINT_LIMIT("CMDQ_IOCTL_ALLOC_WRITE_ADDRESS copy_to_user failed\n");
	...
	case CMDQ_IOCTL_FREE_WRITE_ADDRESS:
		do {
			...
			if (copy_from_user(&freeReq, (void *)param, sizeof(freeReq))) {
				CMDQ_ERR_PRINT_LIMIT("CMDQ_IOCTL_FREE_WRITE_ADDRESS copy_from_user failed\n");
	...
	case CMDQ_IOCTL_READ_ADDRESS_VALUE:
		do {
			...
			if (copy_from_user(&readReq, (void *)param, sizeof(readReq))) {
				CMDQ_ERR_PRINT_LIMIT("CMDQ_IOCTL_READ_ADDRESS_VALUE copy_from_user failed\n");
	...
	case CMDQ_IOCTL_QUERY_CAP_BITS:
		do {
			...
			if (copy_to_user((void *)param, &capBits, sizeof(int))) {
				CMDQ_ERR_PRINT_LIMIT("Copy capacity bits to user space failed\n");
	...
	case CMDQ_IOCTL_SYNC_BUF_HDCP_VERSION:
		...
		do {
			...
			if (0 != status)
				CMDQ_ERR_PRINT_LIMIT("cmdq_sec_sync_handle_hdcp_unlock returns %d\n", status);

		} while (0);
#else
		CMDQ_ERR_PRINT_LIMIT("SVP not supported\n");
		return -IOCTL_RET_SVP_NOT_SUPPORT;
#endif
		break;
	default:
		CMDQ_ERR_PRINT_LIMIT("unrecognized ioctl 0x%08x\n", code);
		CMDQ_ERR_PRINT_LIMIT("CMDQ_IOCTL_LOCK_MUTEX:0x%08lx sizeof(int) = %ld\n",
			CMDQ_IOCTL_LOCK_MUTEX, sizeof(int));
		CMDQ_ERR_PRINT_LIMIT("CMDQ_IOCTL_UNLOCK_MUTEX:0x%08lx sizeof(int) = %ld\n",
			CMDQ_IOCTL_UNLOCK_MUTEX, sizeof(int));
		CMDQ_ERR_PRINT_LIMIT("CMDQ_IOCTL_EXEC_COMMAND:0x%08lx sizeof(struct cmdqCommandStruct) = %ld\n",
			CMDQ_IOCTL_EXEC_COMMAND, sizeof(struct cmdqCommandStruct));
		CMDQ_ERR_PRINT_LIMIT("CMDQ_IOCTL_QUERY_USAGE:0x%08lx sizeof(struct cmdqUsageInfoStruct) = %ld\n",
			CMDQ_IOCTL_QUERY_USAGE, sizeof(struct cmdqUsageInfoStruct));
		CMDQ_ERR_PRINT_LIMIT("CMDQ_IOCTL_ASYNC_JOB_EXEC:0x%08lx sizeof(struct cmdqJobStruct) = %ld\n",
			CMDQ_IOCTL_ASYNC_JOB_EXEC, sizeof(struct cmdqJobStruct));
		CMDQ_ERR_PRINT_LIMIT("CMDQ_IOCTL_ASYNC_JOB_WAIT_AND_CLOSE:0x%08lx sizeof(struct cmdqJobResultStruct) = %ld\n",
			CMDQ_IOCTL_ASYNC_JOB_WAIT_AND_CLOSE, sizeof(struct cmdqJobResultStruct));
		CMDQ_ERR_PRINT_LIMIT("CMDQ_IOCTL_ALLOC_WRITE_ADDRESS:0x%08lx sizeof(struct cmdqWriteAddressStruct) = %ld\n",
			CMDQ_IOCTL_ALLOC_WRITE_ADDRESS, sizeof(struct cmdqWriteAddressStruct));
		CMDQ_ERR_PRINT_LIMIT("CMDQ_IOCTL_FREE_WRITE_ADDRESS:0x%08lx sizeof(struct cmdqWriteAddressStruct) = %ld\n",
			CMDQ_IOCTL_FREE_WRITE_ADDRESS, sizeof(struct cmdqWriteAddressStruct));
		CMDQ_ERR_PRINT_LIMIT("CMDQ_IOCTL_READ_ADDRESS_VALUE:0x%08lx sizeof(struct cmdqReadAddressStruct) = %ld\n",
			CMDQ_IOCTL_READ_ADDRESS_VALUE, sizeof(struct cmdqReadAddressStruct));
		CMDQ_ERR_PRINT_LIMIT("CMDQ_IOCTL_QUERY_CAP_BITS:0x%08lx sizeof(int) = %ld\n",
			CMDQ_IOCTL_QUERY_CAP_BITS, sizeof(int));
		CMDQ_ERR_PRINT_LIMIT("CMDQ_IOCTL_SYNC_BUF_HDCP_VERSION:0x%08lx sizeof(struct cmdqSyncHandleHdcpStruct) = %ld\n",
...
static long cmdq_ioctl_compat(struct file *pFile, unsigned int code, unsigned long param)
{
	...
	case CMDQ_IOCTL_LOCK_MUTEX:
		CMDQ_ERR_PRINT_LIMIT("[COMPAT]deprecated ioctl 0x%08x\n", code);
		return -IOCTL_RET_DEPRECATE_LOCK_MUTEX;
	case CMDQ_IOCTL_UNLOCK_MUTEX:
		CMDQ_ERR_PRINT_LIMIT("[COMPAT]deprecated ioctl 0x%08x\n", code);
		return -IOCTL_RET_DEPRECATE_UNLOCK_MUTEX;
	default:
		CMDQ_ERR_PRINT_LIMIT("[COMPAT]unrecognized ioctl 0x%08x\n", code);
		CMDQ_ERR_PRINT_LIMIT("CMDQ_IOCTL_LOCK_MUTEX:0x%08lx sizeof(int) = %ld\n",
			CMDQ_IOCTL_LOCK_MUTEX, sizeof(int));
		CMDQ_ERR_PRINT_LIMIT("CMDQ_IOCTL_UNLOCK_MUTEX:0x%08lx sizeof(int) = %ld\n",
			CMDQ_IOCTL_UNLOCK_MUTEX, sizeof(int));
		...
		CMDQ_ERR_PRINT_LIMIT("CMDQ_IOCTL_SYNC_BUF_HDCP_VERSION:0x%08lx sizeof(struct cmdqSyncHandleHdcpStruct) = %ld\n",
			CMDQ_IOCTL_SYNC_BUF_HDCP_VERSION, sizeof(struct cmdqSyncHandleHdcpStruct));
		CMDQ_ERR_PRINT_LIMIT("CMDQ_IOCTL_GET_FENCE_FD:0x%08lx sizeof(int) = %ld\n",
			CMDQ_IOCTL_GET_FENCE_FD, sizeof(int));
...
static irqreturn_t cmdq_irq_handler(int IRQ, void *pDevice)
{
	...
	do {
		if (cmdq_dev_get_irq_id() == IRQ) {
			...
			CMDQ_ERR_PRINT_LIMIT("receive secure IRQ %d in NWD\n", IRQ);
		}
	} while (0);
...
static int cmdq_probe(struct platform_device *pDevice)
{
	...
	/* init cmdq context */
	cmdqCoreInitialize();
	cmdqFenceCreateTimeLine();
	status = alloc_chrdev_region(&gCmdqDevNo, 0, 1, CMDQ_DRIVER_DEVICE_NAME);
	if (status != 0)
		CMDQ_ERR_PRINT_LIMIT("Get CMDQ device major number(%d) failed(%d)\n", gCmdqDevNo, status);
	else
	...
	if (status != 0) {
		CMDQ_ERR_PRINT_LIMIT("Register cmdq driver irq handler(%d) failed(%d)\n", gCmdqDevNo, status);
		return -EFAULT;
	}
	...
	if (NULL == proc_create(CMDQ_DRIVER_DEVICE_NAME, 0644, NULL, &cmdqOP)) {
		CMDQ_ERR_PRINT_LIMIT("CMDQ procfs node create failed\n");
		return -EFAULT;
	}
#ifdef CMDQ_OF_SUPPORT
...
static int __init cmdq_init(void)
{
	...
	if (0 != status) {
		CMDQ_ERR_PRINT_LIMIT("register fb notifier error:%d\n", status);
		return -ENODEV;
	}
	...
	if (0 != status) {
		CMDQ_ERR_PRINT_LIMIT("Failed to register the CMDQ driver(%d)\n", status);
		return -ENODEV;
	}
	...
	if (0 != status) {
		CMDQ_ERR_PRINT_LIMIT("Failed to register_pm_notifier(%d)\n", status);
		return -ENODEV;
	}
...
static void __exit cmdq_exit(void)
{
	...
	if (0 != status)
		CMDQ_ERR_PRINT_LIMIT("Failed to unregister_pm_notifier(%d)\n", status);
