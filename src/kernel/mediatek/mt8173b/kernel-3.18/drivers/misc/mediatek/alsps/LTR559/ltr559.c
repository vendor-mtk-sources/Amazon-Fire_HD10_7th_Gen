...
#define LTR559_CAL_FILE   "/data/als_cal_data.bin"
//#define DEMO_BOARD

#define ALS_DEFULT_TRANSMITTANCE    8000
#define ALS_CALI_VALUE      400

#define LTR559_DATA_BUF_NUM 1

#define ALS_WINFAC1			48
#define ALS_WINFAC2			84
#define ALS_WINFAC3			84
...
struct alsps_hw alsps_cust_ltr559;
static struct alsps_hw *hw = &alsps_cust_ltr559;
...
static int als_gainrange = 1;
static int als_cali = 8000;
...
static int ltr559_als_read(struct i2c_client *client, u16* data)
{
	...
	if (ratio < 45) {
		luxdata_int = ((32987 * alsval_ch0) + (20561 * alsval_ch1)) / als_gainrange;
		luxdata_int = (luxdata_int * ALS_WINFAC1/10)/als_transmittance;
		//luxdata_int = (((17743 * alsval_ch0) + (11059 * alsval_ch1)) / als_gainrange) / als_transmittance;
	}
	else if ((ratio < 64) && (ratio >= 45)) {
		luxdata_int = ((42785 * alsval_ch0) - (19548 * alsval_ch1)) / als_gainrange;
		luxdata_int = (luxdata_int*ALS_WINFAC2/10)/ als_transmittance;
		//luxdata_int = (((42785 * alsval_ch0) - (19548 * alsval_ch1)) / als_gainrange) / als_transmittance;
	}
	else if ((ratio < 85) && (ratio >= 64)) {
		luxdata_int = ((7822 * alsval_ch0) + (1564 * alsval_ch1)) / als_gainrange;
		luxdata_int = (luxdata_int*ALS_WINFAC3/10)/ als_transmittance;
		//luxdata_int = (((5926 * alsval_ch0) + (1185 * alsval_ch1)) / als_gainrange) / als_transmittance;
	}
	else {
		luxdata_int = 0;
	}
...
static int ltr559_get_als_value(struct ltr559_priv *obj, u16 als)
{
	...
	if(!invalid)
	{
		...
		return als;		//obj->hw->als_value[idx];
	}
	else
	{
...
static bool als_store_cali_transmittance_in_file (const char *filename, unsigned int value)
{
	...
	mm_segment_t fs;
	char w_buf[LTR559_DATA_BUF_NUM * sizeof (unsigned int) * 2 + 1] =
		{ 0 }, r_buf[LTR559_DATA_BUF_NUM * sizeof (unsigned int) * 2 + 1] =
	{
	0};
	int i;
	char *dest = w_buf;
	...
	for (i = 0; i < LTR559_DATA_BUF_NUM; i++)
	{
		...
		printk ("w_buf: %s \n", w_buf);
		cali_file->f_op->write (cali_file, (void *) w_buf, LTR559_DATA_BUF_NUM * sizeof (unsigned int) *2 + 1, &cali_file->f_pos);
		cali_file->f_pos = 0x00;
		cali_file->f_op->read (cali_file, (void *) r_buf, LTR559_DATA_BUF_NUM * sizeof (unsigned int) *2 + 1, &cali_file->f_pos);

		for (i = 0; i < LTR559_DATA_BUF_NUM * sizeof (unsigned int) * 2 + 1; i++)
		{
			if (r_buf[i] != w_buf[i])
			{
...
static ssize_t ltr559_show_alscali(struct device_driver *ddri, char *buf)
{
	...
	if (res > 0){
		...
		printk (KERN_INFO "%s: alscali als read old lux:=%d\n", __func__, res);
		printk (KERN_INFO "%s: alscali als read old als_cali:=%d\n", __func__, als_cali);

		als_cali = (als_transmittance * res / ALS_CALI_VALUE);

		als_transmittance = als_cali;

		printk (KERN_INFO "%s: alscali als read new als_cali:=%d\n", __func__, als_cali);
		printk (KERN_INFO "%s: alscali als read new lux:=%d\n", __func__, res);

		result = als_store_cali_transmittance_in_file (LTR559_CAL_FILE, als_cali);
		printk (KERN_INFO "%s: result:=%d\n", __func__, result);
		//calculate lux base on calibrated transmittance
	}

	return snprintf(buf, PAGE_SIZE, "%d, %d, %s\n", res, als_cali, result ? "PASSED" : "FAIL");
}
/*----------------------------------------------------------------------------*/
static ssize_t ltr559_show_alstest(struct device_driver *ddri, char *buf)
{
	int res;
...
static DRIVER_ATTR(alscali, S_IWUSR | S_IRUGO, ltr559_show_alscali, NULL);
static DRIVER_ATTR(alstest, S_IWUSR | S_IRUGO, ltr559_show_alstest, NULL);
/*----------------------------------------------------------------------------*/
...
int ltr559_setup_eint(struct i2c_client *client)
{
	...
	ltr559_obj->irq_node = of_find_compatible_node(NULL, NULL, "mediatek,als_ps");
...
/*----------------------------------for factory mode test---------------------------------------*/
		...
		//case ALSPS_WRITE_CALIBRATE:     //\BB\F1ȡ psУ׼\CA\FD\BE\DD
...
static int  ltr559_local_init(void)
{
	...
	if (als_cali > 0 && als_cali <= 65535)
	{
		...
		als_transmittance = ALS_DEFULT_TRANSMITTANCE;
	}
