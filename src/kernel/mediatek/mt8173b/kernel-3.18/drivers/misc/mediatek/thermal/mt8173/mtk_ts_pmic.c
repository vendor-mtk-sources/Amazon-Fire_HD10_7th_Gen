...
{
	tspmic_dprintk("[mtktspmic_remove]\n");
	mtktspmic_unregister_thermal();
	mtktspmic_unregister_cooler();
}
...
struct platform_device mtk_thermal_pmic_device = {
	.name = "MT6391_THERMAL",
	.id = -1,
};
...
static int __init mtktspmic_init(void)
{
	...
	ret = platform_device_register(&mtk_thermal_pmic_device);

	if (ret) {
		pr_err("[mtktspmic_init] Unable to device register(%d)\n", ret);
		return ret;
	}

	ret = platform_driver_register(&mtk_thermal_pmic_driver);

	if (ret) {
		pr_err("[mtktspmic_init] Unable to device driver(%d)\n", ret);
		return ret;
	}
