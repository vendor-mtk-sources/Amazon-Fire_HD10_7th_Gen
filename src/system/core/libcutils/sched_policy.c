...
#include <cutils/iosched_policy.h> //fosmod_performance_bug_fix oneline
...
#include <sched.h>
#include <sys/prctl.h>
#include <cutils/properties.h> // fosmod_performance_bug_fix oneline
...
static int add_tid_to_cgroup(int tid, SchedPolicy policy)
{
    ...
    case SP_AUDIO_APP:
    case SP_AUDIO_SYS:
    case SP_TOP_APP:  // fosmod_eol_backport oneline
        fd = fg_cgroup_fd;
        break;
    default:
        fd = -1;
...
int get_sched_policy(int tid, SchedPolicy *policy)
{
    ...
    if (__sys_supports_schedgroups) {
        ...
        // fosmod_eol_backport begin
        } else if (!strcmp(grpBuf, "top-app")) {
            *policy = SP_TOP_APP;
        // fosmod_eol_backport end
        } else {
            errno = ERANGE;
            return -1;
        }
...
int set_sched_policy(int tid, SchedPolicy policy)
{
    ...
    // fosmod_performance_bug_fix begin
    unsigned long int timerslack_bg_ns = TIMER_SLACK_BG;
    char prop_buf[PROPERTY_VALUE_MAX] = {'\0',};
    int len = property_get("ro.pr.timerslack.bg", prop_buf, "");
    if (len > 0) {
        timerslack_bg_ns = strtoul(prop_buf, NULL, 0);
        if (timerslack_bg_ns == 0) {
            timerslack_bg_ns = TIMER_SLACK_BG;
        }
    }
    prctl(PR_SET_TIMERSLACK_PID, policy == SP_BACKGROUND ? timerslack_bg_ns : 0, tid);

    /* Set lowest best effort I/O scheduling for bg, default otherwise.
       This is to reduce impact of background I/O on fg app resource loading
    */
    ...
    if (android_set_ioprio(tid, clazz, ioprio))
        SLOGW("Failed to set io priority for tid %d (%s)", tid, strerror(errno));
    // fosmod_performance_bug_fix end
    return 0;
}
...
const char *get_sched_policy_name(SchedPolicy policy)
{
    ...
    static const char * const strings[SP_CNT] = {
       ...
       [SP_AUDIO_APP]  = "aa",
       [SP_AUDIO_SYS]  = "as",
       [SP_TOP_APP]    = "ta",  // fosmod_eol_backport oneline
    };
